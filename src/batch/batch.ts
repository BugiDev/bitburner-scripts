...
import { getBatchHWGWConfig, SingleBatchHWGWConfig } from '/batch/batch-hwgw-config';
...
async function executeBatch(
  ns: NS,
  targetServer: string,
  batchConfig: SingleBatchHWGWConfig,
  id: string | number
) {
  const delays = calculateDelays(ns, targetServer);

  await executeWeakScript(
    ns,
    batchConfig.weakHackServer,
    targetServer,
    batchConfig.weakenThreadsNeededForHack,
  ...
  await executeWeakScript(
    ns,
    batchConfig.weakGrowServer,
    targetServer,
    batchConfig.weakenThreadsNeededForGrow,
    `${targetServer}-weak-grow-${id}`,
    delays.weakGrow
  );

  await executeGrowScript(
    ns,
    batchConfig.growServer,
    targetServer,
    batchConfig.threadsToGrow,
    `${targetServer}-grow-${id}`,
    delays.grow
  );

  await executeHackScript(
    ns,
    batchConfig.hackServer,
    targetServer,
    batchConfig.threadsToHack,
