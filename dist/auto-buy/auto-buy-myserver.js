...
async function purchaseServer(ns, ram, debug = false) {
 ...
 * @param debug
 */
async function autoPurchaseServer(ns, debug = false) {
 ...
 * @param debug
 */
function getNextServerLevel(ns) {
    const purchasedServers = ns.getPurchasedServers();
    const purchaseServerLimit = ns.getPurchasedServerLimit();
    if (purchasedServers.length === 0) {
    ...
    const areAllServerMaxedOut = purchasedServers.every((server) => {
        const serverMaxRam = ns.getServerMaxRam(server);
        const serverLevel = Math.log(serverMaxRam) / Math.log(2);
